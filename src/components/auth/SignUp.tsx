...
interface Props { onClose: () => void; onSwitchToLogin: () => void; onSuccess: (role: string) => void; }

export function SignUp({ onClose, onSwitchToLogin, onSuccess }: Props) {
  const [form, setForm] = useState({ name: "", email: "", phone: "", password: "", role: "customer" as string });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await signUp(form.email, form.password, form.name, form.phone, form.role as any);
      onSuccess(form.role);
    } catch (e: any) {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      ...
      <motion.div initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.96 }}
        ...
        <h2 className="text-2xl font-bold text-fg mb-1">Create account</h2>
        <p className="text-sm text-muted-fg mb-6">Choose your role and get started.</p>
        ...
        <p className="text-sm text-muted-fg text-center mt-6">
          ...
          <button type="button" onClick={onSwitchToLogin} className="text-fg font-medium hover:text-primary transition-colors">Sign in</button>
        </p>
      </motion.div>
    </motion.div>
  );
