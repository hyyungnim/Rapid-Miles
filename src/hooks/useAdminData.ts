...
export function useAdminData() {
  ...
  const fetchData = useCallback(async () => {
    ...
    setTopDrivers([]);
    setRevenue({ today: fbRevenue, week: fbRevenue, month: fbRevenue, allTime: fbRevenue, monthly: Array(12).fill(0) });
  ...
  useEffect(() => {
    ...
    const onStorage = (e: StorageEvent) => {
      if (e.key === "rm_bookings") fetchData();
    };
    window.addEventListener("storage", onStorage);
    return () => window.removeEventListener("storage", onStorage);
  }, [fetchData]);

  return { stats, recentDeliveries, allBookings, topDrivers, revenue, analytics, loading, refresh: fetchData };
}
